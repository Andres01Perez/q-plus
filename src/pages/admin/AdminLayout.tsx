...
import { 
  Home, Building, Settings, Menu, X, LogOut, ChevronRight,
  LayoutGrid, PlusCircle, List
} from 'lucide-react';
...
const AdminLayout = () => {
  ...
  const menuItems = [
    ...
    { 
      ...
      children: [
        { label: 'Todas', path: '/admin/propiedades', icon: List },
        { label: 'Nueva', path: '/admin/propiedades/nueva', icon: PlusCircle },
      ]
    },
    { 
      label: 'Configuración', 
      ...
      children: [
        { label: 'Bloques', path: '/admin/configuracion/bloques', icon: LayoutGrid },
      ]
    },
  ];
            ...
            {menuItems.map((item) => (
              <div key={item.path} className="mb-1">
                ...
                >
                  ...
                  {item.children && (
                    <ChevronRight className={`h-4 w-4 ml-auto transition-transform ${isActive(item.path) ? 'rotate-90' : ''}`} />
                  )}
                </Link>
